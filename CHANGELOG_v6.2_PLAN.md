...
| 版本 | 状态 | 内容 |
|------|------|------|
| **v6.1.3** | ✅ 锁版 | 回归测试 10 个样例，`fmtAnchor()`，P1/P2/P3 `Math.round()` |
| **v6.1.3-patch** | ⬜ 下一步（文案/字段增强） | 区分 `exact_domain_historical_sale` vs `comparable_sale`，加 `anchor_note` |
| **v6.2** | ⏸ 暂缓 | ANCHORS Price Floor Guard — 等更多真实样本后再启动 |

### v6.1.3 锁版说明

- commit `710af1d` 为锁版候选，无阻塞问题
- `scoreFloor` 是最低保护分，最终分会被六维加权结果抬高（1888.com 80→83，12345.com 70→78，3m.com 72→85）；这是预期行为，不是 bug
- 唯一待收口的是 GOKA.com 锚点说明（非阻塞），在下一个 patch 中处理

---

## v6.1.3-patch：anchor_relation 字段区分

### 背景

当前 ANCHORS 表中，zkp.com / 62.com 等用自身历史成交做参考（本域名历史锚点）；GOKA.com 等也属于同类。  
AI 看到物料包时，必须能区分：
- **本域名历史锚点**（exact_domain_historical_sale）— 可靠，但可能是历史低价/急售
- **相似域名参考成交**（comparable_sale）— 独立市场样本，参考价值不同

否则 AI 可能误以为有多个独立市场样本，拔高估值置信度。

### 新增字段

在物料包 ANCHORS 区块中新增两个字段：

```json
{
  "anchor_relation": "exact_domain_historical_sale",
  "anchor_note": "该锚点为本域名历史成交/报价记录，不是独立相似域名样本；可作为历史价格参考，但仍需结合当前市场与买家情况复核。"
}
```

| `anchor_relation` 值 | 含义 |
|---|---|
| `exact_domain_historical_sale` | 本域名自身历史成交或历史报价 |
| `comparable_sale` | 独立相似域名市场成交（独立数据点） |
...
{
  "domain": "GOKA.com",
  "asset_class": "LLLL_PRONOUNCEABLE_COM",
  "anchor_usd": 399995,
  "anchor_relation": "exact_domain_historical_sale",
  "anchor_note": "该锚点为本域名历史成交/报价记录，不是独立相似域名样本；可作为历史价格参考，但仍需结合当前市场与买家情况复核。"
}
...
- 只新增字段，**不改任何分类/评分/定价逻辑**
- `buildPack()` 输出时带入这两个字段
- UI 可选择在锚点说明行展示 `anchor_relation` 标签（小灰字）

---

## v6.2（暂缓）：ANCHORS Price Floor Guard

> ⏸ 以下为完整设计规范，待真实样本充足后启动。
...
*规划日期：2026-06-19*  
*锁版确认：2026-06-19（commit 710af1d）*  
*基于：v6.1.3 A阶段实测报告*
